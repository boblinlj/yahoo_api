CREATE TABLE `yahoo_most_shorted_stocks` (
   ...
   `lastCloseTevEbitLtm` double DEFAULT NULL,
   `lastClosePriceToNNWCPerShare` double DEFAULT NULL,
   ...
   `earningsTimestamp` date DEFAULT NULL,
   ...
   `newListingDate` date DEFAULT NULL,
   ...
   `exchangeTransferDate` date DEFAULT NULL,
   `prevExchange` varchar(45) DEFAULT NULL,
   `updated_dt` date NOT NULL,
   PRIMARY KEY (`date_id`,`stock`,`updated_dt`),
   KEY `idx_yahoo_most_shorted_stocks_stock` (`stock`),
   KEY `idx_yahoo_most_shorted_stocks_updated_dt` (`updated_dt`)
